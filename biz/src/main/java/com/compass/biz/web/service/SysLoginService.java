...
//        }
        ...
        catch (Exception e)
        {
            ...
            {
                AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, CompassConstants.LOGIN_FAIL, MessageUtils.message("user.password.not.match")));
                throw new UserPasswordNotMatchException();
            }
            else
            {
                AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, CompassConstants.LOGIN_FAIL, e.getMessage()));
                throw new ServiceException(e.getMessage());
            }
        }
        AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, CompassConstants.LOGIN_SUCCESS, MessageUtils.message("user.login.success")));
    ...
    public void validateCaptcha(String username, String code, String uuid)
    {
        ...
        if (captcha == null)
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, CompassConstants.LOGIN_FAIL, MessageUtils.message("user.jcaptcha.expire")));
            throw new CaptchaExpireException();
        }
        if (!code.equalsIgnoreCase(captcha))
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, CompassConstants.LOGIN_FAIL, MessageUtils.message("user.jcaptcha.error")));
